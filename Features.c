...
/*---------------------------------------------------------------------------*/
int FindFeatures(char* fileName)
{
	int i, filterIndex;
	int radius;
	PIXEL *pPixel;
	...
	for (i= 1; i<g_locations; i++)
	{
		...
		// IMPORTANT: For some unusual reason, the QDED code doesn't start at the smallest col and row. I'm not entirely sure why, but this requires you to manually adjust the
		// 2 variables below to make sure it starts writing AT the smallest 2 variables. This will require 2 runs of the code to see the smallest variable, and to get it written as
		// the first thing.
		if(Location[i].c == 3 && Location[i].r == 3) {
			startWrite = 1;
		}
