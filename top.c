...
int ConeDetect(char* image_name)
{
	...
	*/
	...
	printf("post RGB\n");
	WriteALL(width, height, Intensity, "Gray_cone.pgm", 5);
	WriteALL(width, height, Image, "RGB_cone.ppm", 6);
	...
	*/
	...
	while(dir) {
		...
		if(de->d_name[0] == '.') {
			...
			//printf("%s\n", de->d_name);
			char dir_name[256] = "./images/";
			char txt_name[256] = "./text/";
			strcat(dir_name, de->d_name);

			de->d_name[strlen(de->d_name) - 1] = 't';
			de->d_name[strlen(de->d_name) - 2] = 'x';
			de->d_name[strlen(de->d_name) - 3] = 't';
			strcat(txt_name, de->d_name);
			printf("%s\n", txt_name);
			ReadPGM(dir_name);
			GetGrid();
			FindFeatures(txt_name);
		}
	}
	closedir(dir);

	/*char image_name[256] = "./images/";	
	char* imagetxt_name;
	...
	imagetxt_name = "./text/stop.txt";
	
	ReadPGM(image_name);
	GetGrid();
	getFilter(16);
	FindFeatures(imagetxt_name);*/
